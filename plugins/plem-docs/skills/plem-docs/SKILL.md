---
name: plem-docs
description: "Provides the agent with the PLEM robot-control platform's Developer Manual — fetched live from the Depot server, version-matched to the installed PLEM build when run on the device, otherwise the latest published docs (Context7-style). Use when the user works with a PLEM / Neuromeka Indy robot: trajectory control and FollowJointTrajectory goals, the ROS 2 topics, actions, and namespaces a PLEM robot exposes, launch files and the plem TUI (freedrive, brake), single- or multi-robot setup, emergency stop and recovery, safety, MoveIt motion planning, writing a custom PLEM controller plugin, the platform architecture, or initializing a PLEM-based ROS 2 project (.repos, colcon). Recognize robot model names like indy7 or indy12 as PLEM context even when the user doesn't say 'PLEM' or 'docs'. This is PLEM-specific — generic ROS 2 or ZED-SDK questions belong to other tools."
argument-hint: "[topic...]"
---

# plem-docs — 설치 버전에 맞는 PLEM 매뉴얼 (라이브)

디바이스(Jetson)에 설치된 PLEM 버전에 **일치하는** PLEM Developer Manual을 Depot에서 실시간으로
가져온다. 디바이스 밖(예: 개발 랩탑)에서는 설치된 `plem` deb 가 없어 `latest` 를 받으므로, 특정
버전이 필요하면 `PLEM_DOCS_VERSION` 으로 핀한다. 정적 번들이 아니라 라이브 서빙이므로 stale 되지
않는다 — Context7과 동일한 발상이다.
...
ver="${PLEM_DOCS_VERSION:-}"                              # 명시 핀이 있으면 우선
if [ -z "$ver" ]; then
  ver="$(dpkg-query -W -f='${Version}' plem 2>/dev/null)" # 없으면 plem 메타패키지 = 버전 SSoT
...
- **버전 결정 순서**: `PLEM_DOCS_VERSION`(명시 핀) → 설치된 `plem` deb(디바이스 자동 감지) → `latest`. `dpkg` 는 디바이스(Jetson)에서만 resolve 되므로, 코딩 에이전트가 주로 도는 **개발 랩탑에서는 `latest` 가 기본**이다. 타깃 Jetson 의 버전으로 고정하려면 `PLEM_DOCS_VERSION=0.2.10` 처럼 핀한다.
...
- **버전 정합 우선**: 디바이스에 설치된 PLEM과 *같은 버전*의 문서를 근거로 답한다. 디바이스 밖에서 실행해 `latest` 로 폴백했거나 버전이 다르면 그 사실을 밝힌다 (특정 버전이 필요하면 `PLEM_DOCS_VERSION` 으로 핀).
