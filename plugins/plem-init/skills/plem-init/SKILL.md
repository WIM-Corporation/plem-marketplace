...
이 경우 wim_control 의 통합 `./packaging/zed-setup/zed-setup install` 로 SDK + 드라이버 +
wrapper workspace 를 한 번에 설치할 수 있다. 상세: `packaging/zed-setup/README.md`.
...
### Step 3.5: Camera Setup (if camera selected)

If a Stereolabs camera was selected, 설치는 **wim_control 의 통합 `zed-setup` CLI** 에 위임한다.
본 skill 은 사용자 프로젝트용 **자산 복사** 와 **호출 안내** 만 담당한다.

**시스템 환경 설치** (ROS 2 + SDK + GMSL2 드라이버 + wrapper workspace):

```bash
# wim_control 저장소 경로에서 실행 (정확한 경로는 환경에 따라 다름)
./packaging/zed-setup/zed-setup install --preset=<PRESET>
```

카메라 선택값 → preset 자동 매핑:

| 선택 카메라 | `--preset` |
|-------------|------------|
| `zedxm`, `zedx` | `gmsl-zed-x-stereo` |
| `zedxonegs`, `zedxone4k` | `gmsl-zed-x-one-mono` |
| USB 카메라 (`zed2i`, `zed2` 등) | `usb-zed2` |

대화형으로 실행하려면 `zed-setup` 을 인자 없이 실행 (권장). `zed-setup` 이 Jetson 감지,
호환성 가드 (Mono/AGX 조합 차단 등), 커널 Gate (PREEMPT/PREEMPT_RT 자동 매칭),
extlinux.conf 백업, 리부트 정책 (auto/manual/resume), 상태 파일 기반 재개를 모두 자동 처리한다.

**사용자 프로젝트로 복사할 자산** (skill 이 직접 책임):

```bash
# RViz 시각화 설정 (프로젝트마다 필요한 자산)
mkdir -p scripts/zed/config
cp <skill-dir>/scripts/zed/config/zedxm_display.rviz scripts/zed/config/
```

Where `<skill-dir>` is the plem-init skill directory (the directory containing this SKILL.md).

**설치 후 개발 가이드**: `references/zed-dev-quickstart.md` — QoS 매칭, 토픽 subscribe,
VisionInspection action server 스캐폴딩, YOLO 연동 등.

**상세 설치 문서**: `packaging/zed-setup/README.md` — 호환성 매트릭스 (Jetson × card × camera × L4T),
V1–V7 severity-based 정책, 커널 Gate, 상태 파일, 리부트 정책, 트러블슈팅.

**운영 중 복구 가이드**: `references/zed-driver-setup.md` — Isaac ROS GXF, zed_x_daemon 복구,
ISP daemon 재시작, libnvisppg.so 복원 같은 특수 상황 (일상 설치 범위 밖).
...
`camera_model:=zedxm` only adds TF frames to the URDF. Actual camera image streaming requires a separate driver installation.
...
- [ ] `scripts/zed/config/zedxm_display.rviz` copied (if camera selected) — RViz 시각화 자산
- [ ] ZED 환경 설치 안내 포함 — `./packaging/zed-setup/zed-setup` 호출 가이드 (if camera selected)
...
카메라 활용 (ZED):
  ...
  카메라 이미지를 사용하려면 ZED SDK + (GMSL2 시) ZED Link 드라이버 + zed-ros2-wrapper
  workspace 가 필요합니다. wim_control 의 통합 CLI 로 한 번에 설치 가능:

    # 대화형 (권장 — 환경 자동 감지 + 호환성 가드)
  ...
  설치 상태 확인 / 커널 정합성 진단:
    ./packaging/zed-setup/zed-setup status
    ./packaging/zed-setup/zed-setup doctor-kernel
...
따라서 SDK 설치 → wrapper 빌드를 `packaging/zed-setup/` 통합 CLI 가 순차 관리한다.
