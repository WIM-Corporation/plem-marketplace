...
> 상세 문법은 `/zed-docs` 스킬 참조 (파라미터 오버라이드, 토픽명, QoS 등).
...
- **YOLO integration**: See `zed-docs 스킬의 references/yolo-integration.md` for ZED + YOLO 3D detection (invoke with `/zed-docs yolo`)
- **API reference**: See `zed-docs 스킬의 references/ros2-api-reference.md` for complete topic/service/parameter list (invoke with `/zed-docs api`)
