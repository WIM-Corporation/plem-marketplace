---
description: "plem 고정형 manipulator에 ZED 카메라를 통합하는 실전 가이드 — TF 결정, depth_stabilization 필수 조합, URDF 2-Layer 패턴, mount YAML, Hand-Eye 캘리브레이션, 멀티카메라 TF, plem_server 연동, rosbridge NaN/CBOR, Object Detection 프레임 주의"
source: "zed-docs/references/robot-integration.md"
---
...
> Eye-on-Hand 구성에서 ZED VIO는 불안정하다 — 핸드의 움직임으로 카메라 좌표가
> 원점에서 수십m 이탈(드리프트)하거나, TF 쿼터니언이 NaN으로 발산하여 rviz가
> OOM 크래시할 수 있다. 독립 테스트에서도 `publish_tf:=false` +
> `pos_tracking_enabled:=false`로 설정하고, rviz Fixed Frame을
> `zed_left_camera_frame`으로 지정하면 안정적으로 시각화할 수 있다.
...
plem 플랫폼에서 `camera_name`은 **역할 기반**으로 설정한다.
벤더명(`zed`)이나 로봇명(`indy`)을 넣지 않는다 -- `/{robot_id}/` 네임스페이스가 이미 로봇별 격리를 보장하기 때문이다.

기본값: `cam`. 멀티카메라 시: `hand_cam`, `front_cam` 등.

```bash
# ZED 드라이버 launch 시 camera_name을 URDF와 일치시킨다
...
ZED 드라이버 launch 시 `/{robot_id}/` namespace 안에서 실행해야 한다.
이렇게 해야 로봇 제어 토픽과 같은 namespace 트리에 속하고, 멀티로봇 시 토픽 충돌이 없다.
...
| 기능 | standalone (기본) | plem 통합 (robot_id=indy) |
|------|-------------------|--------------------------|
| 이미지 | `/zed/zed_node/rgb/color/rect/image` | `/indy/cam/rgb/color/rect/image` |
| Point Cloud | `/zed/zed_node/point_cloud/cloud_registered` | `/indy/cam/point_cloud/cloud_registered` |
| OD | `/zed/zed_node/obj_det/objects` | `/indy/cam/obj_det/objects` |
| OD 서비스 | `/zed/zed_node/enable_obj_det` | `/indy/cam/enable_obj_det` |
| Description | `/zed/zed_description` | `/indy/cam_description` |
| Plane marker | `/zed/plane_marker` | `/indy/plane_marker` |

> **rviz 설정 변경 체크리스트** — `camera_name`/`namespace` 변경 시:
> 1. 데이터 토픽 경로 (패턴 A: `/<camera_name>/zed_node/<topic>`, 패턴 B: `/<namespace>/<camera_name>/<topic>`)
> 2. TF 프레임 이름 (`<camera_name>_camera_link`, `<camera_name>_left_camera_frame` 등)
> 3. Description 토픽 (`/<namespace>/<camera_name>_description`)
> 4. Plane marker 토픽 (`/<namespace>/plane_marker`, node_name 미포함)
> 5. Fixed Frame (TF 체인 루트와 일치해야 함)
>
> rviz는 존재하지 않는 토픽을 구독해도 에러를 표시하지 않으므로 (silent failure), `ros2 node info /rviz`로 확인.

### plem_server 연동

`plem_server/config.py`는 `CAMERA_NAME`을 역할 기반 고정값 `cam`으로 설정한다:

```python
CAMERA_NAME = os.getenv("PLEM_CAMERA_NAME", "cam")
# 기본: /{ROBOT_ID}/cam/obj_det/objects
# 멀티카메라 override: PLEM_CAMERA_NAME=hand_cam
```

`PLEM_CAMERA_NAME` 환경변수로 override 가능. URDF 프레임과 반드시 일치시킨다.
...
## 5. Streaming Bridge (GPU 인코딩 원격 시각화)
...
ros2 run tf2_ros static_transform_publisher \
    ...
    --frame-id link6 --child-frame-id cam_camera_link
...
## 8. Object Detection NaN과 rosbridge 호환성

`zed_msgs/msg/Object`에는 body tracking 전용 필드(`head_position`, `skeleton_2d/3d`)가 항상 포함된다.
**Object Detection 전용 모드에서는 이 필드들이 NaN으로 채워진다.** 이건 ZED SDK 동작이며 비활성화할 수 없다.

### 문제

rosbridge는 Python `json.dumps()`로 메시지를 직렬화하는데, Python json은 NaN을 `NaN` 리터럴로 출력한다.
JavaScript `JSON.parse()`는 NaN을 거부하므로 **전체 메시지 파싱이 실패**한다.
→ obj_det/objects 데이터가 프론트엔드에 도달하지 않음, 3D 바운딩박스 렌더링 불가.

### 해결: CBOR 인코딩

roslib의 Topic 구독 시 `compression: 'cbor'`을 설정하면 rosbridge가 CBOR(바이너리) 인코딩을 사용한다.
CBOR는 IEEE 754 NaN을 네이티브로 지원하므로 파싱 실패 없이 데이터가 전달된다.
...
const topic = new ROSLIB.Topic({
  ...
  compression: 'cbor',  // NaN-safe binary encoding
});
```

### 프레임 주의

`obj_det/objects`의 `header.frame_id`는 `cam_left_camera_frame` (ROS 표준: X 전방)이다.
`cam_left_camera_frame_optical` (광학: Z 전방)과 90도 회전 관계이므로 혼동 주의.
